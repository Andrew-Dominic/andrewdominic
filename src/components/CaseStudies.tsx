      ...
      <div ref={contentWrapperRef} className="flex flex-col lg:h-screen will-change-transform">
                  ...
                  <div className="flex-1 flex flex-col justify-center w-full md:pt-32">
                    ...
                    <div className="flex flex-wrap items-center gap-y-2 mb-6">
                      {project.tags.map((tag, index) => (
                        <span key={tag} className="flex items-center text-[0.65rem] sm:text-xs font-header font-bold text-white uppercase tracking-[0.15em]">
                          {tag}
                          {index < project.tags.length - 1 && <span className="mx-2 text-white/30">•</span>}
                        </span>
                      ))}
                    </div>

                    {/* Description */}
                    <p className="text-sm sm:text-base font-body text-white/60 leading-[1.7] mb-8 max-w-sm">
                      {project.description}
                    </p>
                  ...
                  <div className="card-meta absolute z-20" style={{ left: 'calc(32% + 36vw + 28px)', top: '50%', transform: 'translateY(-50%)', maxWidth: '220px' }}>
                    ...
                    <div className="flex flex-col gap-2 mb-5">
                      {project.tags.map((tag) => (
                        <p key={tag} className="text-xs font-header font-bold text-white uppercase tracking-wide border-b border-white/50 w-fit pb-0.5 cursor-default hover:text-white/60 hover:border-white/30 transition-colors">
                          {tag}
                        </p>
                      ))}
                    </div>
        ...
        <div className="hidden lg:flex relative bg-grit-900 z-10 flex-shrink-0 flex-col">
          ...
          <div className="px-16 py-5 flex justify-between items-center w-full">
            <span ref={counterRef} className="text-base font-poster font-bold text-white/80 tracking-wide">
              [ 01 / {String(projects.length).padStart(2, "0")} ]
            </span>

            <a href="#" className="flex items-center gap-2 text-[0.6rem] font-header font-bold tracking-[0.3em] uppercase text-white/50 hover:text-white transition-colors group/all">
              VIEW ALL PROJECTS
