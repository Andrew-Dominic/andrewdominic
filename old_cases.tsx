...
const projects = [
  ...
  },
  {
    ...
    description: "Autonomous drone control without GPS ΓÇö navigation intelligence bridging low-level computing with real-time dashboard controllers.",
...
export default function CaseStudies() {
  ...
  useEffect(() => {
    ...
    // Cache DOM queries ΓÇö avoid querySelectorAll on every frame
    ...
    const st = ScrollTrigger.create({
      ...
      onUpdate: (self) => {
        ...
        // Move the track ΓÇö single gsap.set call
        gsap.set(track, { x: -totalScrollWidth * progress });

        // Progress bar ΓÇö direct transform for GPU acceleration
        if (progressBar) {
          progressBar.style.transform = `scaleX(${progress})`;
        }

        // Counter update ΓÇö direct DOM mutation, no React state
        ...
        // ΓöÇΓöÇ Differential parallax physics ΓöÇΓöÇ
        // Uses cached DOM refs ΓÇö zero querySelectorAll per frame
        for (let i = 0; i < cardData.length; i++) {
          ...
          // Title: SLOWER ΓÇö lags behind scroll direction
          if (title) {
            title.style.transform = `translateY(-45%) translate3d(${delta * -80}px, 0, 0)`;
          }

          // Photo: FASTER ΓÇö pushes ahead in scroll direction
          if (photo) {
  ...
  return (
    <section id="work" ref={sectionRef} className="relative bg-white overflow-hidden">
      <div className="h-screen flex flex-col">
          ...
          <div ref={trackRef} className="flex h-full" style={{ willChange: 'transform' }}>
            ...
            {projects.map((project) => (
              ...
              >
                ...
                {/*
                  ΓöÇΓöÇ Title ΓÇö BLACK text, positioned left, overlapping into the image ΓöÇΓöÇ
                ...
                */}
                ...
                {/*
                  ΓöÇΓöÇ Photo ΓÇö positioned center-right so title overlaps its left edge ΓöÇΓöÇ
                */}
                <div
                ...
                {/* ΓöÇΓöÇ Tags + Description ΓÇö immediately right of the photo ΓöÇΓöÇ */}
                <div
        ...
        {/* ΓöÇΓöÇ Fixed Bottom Bar with Progress ΓöÇΓöÇ */}
